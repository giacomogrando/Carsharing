...
public class Database {
	...
	public static ArrayList<Noleggio> noleggiPerSocio(Date dataN, String nome) {
		...
		try {
			...
			sql = "SELECT codice_noleggio, auto, socio, inizio,  fine, auto_restituita, nome, cognome FROM noleggi INNER JOIN soci ON socio = soci.cf WHERE inizio = '"+sqlData+"' AND nome = '" +nome+"' ";
			...
			while (rs.next() == true) {
				Noleggio a = new Noleggio(rs.getInt("codice_noleggio"), rs.getString("auto"), rs.getString("nome"), rs.getString("cognome"),
	...
	public static ArrayList<Noleggio> nuovoNoleggio(Date inizio, Date fine) {
		ArrayList<Noleggio> elenco = new ArrayList<Noleggio>();
		Connection cn;
		Statement st;
		String sql;
		...
		try {
			...
			/*String nome = StringEscapeUtils.escapeSql(n.getNome());
			String cognome = StringEscapeUtils.escapeSql(n.getCognome());*/
			...
			sql = "SELECT auto FROM noleggi WHERE inizio < '"+sqlDataI+"' AND ";
			System.out.println(sql);
			// ________________________________query
